...
import { acceptLinkingCodeSchema, emailInviteSchema, linkingCodesQuerySchema, linkingLinksQuerySchema } from '@tmjconnect/shared';
...
export function linkingRouter(container: Container) {
  ...
  router.get('/codes', authorize('provider'), validate(linkingCodesQuerySchema, 'query'), async (req, res, next) => {
    try {
      const { limit, offset, sortBy, sortOrder } = parseListQuery(req.query);
