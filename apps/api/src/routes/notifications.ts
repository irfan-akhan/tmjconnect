...
export function notificationsRouter(container: Container) {
  ...
  router.get('/', validate(notificationListQuerySchema, 'query'), async (req, res, next) => {
    try {
      const { cursor, limit } = parseCursorPagination(req.query);
      const { items, hasMore, unread_count } = await List.execute(container, { userId: req.user!.id, cursor, limit });
  ...
  router.patch('/read-all', async (req, res, next) => {
    try {
      await MarkAllRead.execute(container, { userId: req.user!.id });
      res.status(204).send();
    } catch (err) { next(err); }
  });
  ...
  router.patch('/:id/read', async (req, res, next) => {
    try {
      res.json({ data: await MarkOneRead.execute(container, { userId: req.user!.id, notifId: req.params.id }) });
    } catch (err) { next(err); }
  });
