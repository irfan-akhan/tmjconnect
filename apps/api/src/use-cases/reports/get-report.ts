...
export async function execute(deps: Deps, input: GetReportInput) {
  ...
  if (input.role === 'patient') {
    const report = await getReportForPatient(db, input.reportId, input.userId);
  ...
  let report = await getReportForProvider(db, input.reportId, input.userId);
  if (!report) throw new AppError(404, 'NOT_FOUND', 'Report not found.');

  if (report.status === 'submitted') {
    await markReportViewed(db, input.reportId);
    // Re-read to return the actual DB state.
    report = (await getReportForProvider(db, input.reportId, input.userId))!;
  }
