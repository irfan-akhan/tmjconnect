...
type ProfileResponse = {
  data: { id: string; email: string; role: 'provider'; first_name: string; last_name: string };
};

async function fetchProviderUser(): Promise<ProviderUser> {
  const res = await apiFetch<ProfileResponse>('/providers/me');
  if (res.data.role !== 'provider') throw new Error('This portal is for providers only.');
  return {
    id: res.data.id,
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const refresh_token = localStorage.getItem(REFRESH_KEY);
      if (!refresh_token) { setLoading(false); return; }
      try {
  ...
  const login = useCallback<AuthState['login']>(async (email, password, mfaCode) => {
    ...
    if (mfaCode && mfaTokenRef.current) {
      const tokens = await apiFetch<{ access_token: string; refresh_token: string }>(
        '/auth/mfa/verify',
        { method: 'POST', body: { mfa_token: mfaTokenRef.current, code: mfaCode, type: 'totp' } },
      );
      localStorage.setItem(REFRESH_KEY, tokens.refresh_token);
      setAccessToken(tokens.access_token);
      mfaTokenRef.current = null;
    ...
    const res = await apiFetch<{ mfa_required: true; mfa_token: string }>(
      '/auth/provider/login',
