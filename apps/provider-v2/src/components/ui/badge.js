...
const badgeVariants = cva('inline-flex items-center gap-1 rounded-sm border font-mono text-[10px] uppercase tracking-[0.18em] transition-colors', {
    variants: {
        variant: {
            default: 'border-border bg-secondary text-secondary-foreground',
            outline: 'border-border bg-transparent text-foreground',
            muted: 'border-transparent bg-muted text-muted-foreground',
            navy: 'border-transparent bg-navy-600 text-white',
...
export function Badge({ className, variant, size, ...props }) {
    return _jsx("span", { className: cn(badgeVariants({ variant, size }), className), ...props });
}
