...
#
# Passphrase handling: the passphrase file MUST differ from the API's
# BACKUP_PASSPHRASE env var (host-level only, never handed to the container).
# Store a copy in your password manager — restore is impossible without it.
#
...
# (the pipe hides exit codes when `set -o pipefail` is not strict enough
# on some shells). Fail loudly if the file is suspiciously small.
SIZE=$(wc -c < "${DUMP_FILE}")
